...
void TerrainGeneration::render(const glm::mat4& view, const glm::mat4& proj, const vec4& clip_plane) {
...
void TerrainGeneration::renderGUI() {
...
float TerrainGeneration::perlinNoise(float x, float y) {
...
vec2 TerrainGeneration::getCornerVector(int corner) {
	int hash = corner % 4;
	switch (hash) {
...
float TerrainGeneration::fade(float t) {
	return t * t * t * (t * (t * 6 - 15) + 10);
}

float TerrainGeneration::lerp(float x, float p1, float p2) {
	//assert(x >= 0 && x <= 1);

	return p1 + x * (p2 - p1);
}

void TerrainGeneration::genPermutations() {
...
void TerrainGeneration::generateTerrain(int numOctaves) {
...
mesh_builder TerrainGeneration::generatePlane() {
...
vector<vector<float>> TerrainGeneration::erodeTerrainTerraces(vector<vector<float>> heightMap, int size) {
...
vector<vector<float>> TerrainGeneration::erodeTerrainRealistic(vector<vector<float>> heightMap, int size) {
...
float TerrainGeneration::homogeneousfbm(float x, float y, int numOctaves) {
...
float TerrainGeneration::heterogeneousfbm(float x, float y, int numOctaves) {
	float CurrentHeight = 0;
	float weight = 1.0f;
...
float TerrainGeneration::hybridMultifractal(float x, float y, int numOctaves) {
	float CurrentHeight = 0;
	float weight = 1.0f;
